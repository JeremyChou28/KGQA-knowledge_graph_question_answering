...
print("输出构建的样本中的前5个item:", seq_result[0:5])
print(data_type, "中构建的样本数为:", len(seq_result))  # 从training-data中构建了87634个样本，从testing-data中构建了59209个样本
if data_type == 'test':
    with open("./Base5/" + data_type + ".txt", "w", encoding='utf-8') as f:
        f.write("\n".join(seq_result))
        f.close()
elif data_type == "train":
    val_seq_result = seq_result[0:14335]
    with open("./Base5/" + "dev" + ".txt", "w", encoding='utf-8') as f:
        f.write("\n".join(val_seq_result))
        f.close()
    training_seq_result = seq_result[14335:]
    with open("./Base5/" + data_type + ".txt", "w", encoding='utf-8') as f:
