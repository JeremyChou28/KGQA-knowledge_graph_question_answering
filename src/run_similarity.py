...
class BertSim:
    ...
    def inference(self):
        if self.mode is None:
            raise ValueError("Please set the 'mode' parameter")
        infer_examples = self.processor.get_test_examples(args.data_dir)
        infer_file = os.path.join(args.output_dir, "infer.tf_record")
        label_list = self.processor.get_labels()
        self.file_based_convert_examples_to_features(
            infer_examples, label_list, args.max_seq_len, self.tokenizer, infer_file)

        tf.logging.info("***** Running evaluation *****")
        tf.logging.info("  Num examples = %d", len(infer_examples))
        tf.logging.info("  Batch size = %d", self.batch_size)

        infer_input_fn = self.file_based_input_fn_builder(
        ...
        estimator = self.get_estimator()
        result = estimator.evaluate(input_fn=infer_input_fn)

        output_infer_file = os.path.join(args.output_dir, "infer_results.txt")
        with tf.gfile.GFile(output_infer_file, "w") as writer:
            tf.logging.info("***** Infer results *****")
            for key in sorted(result.keys()):
                tf.logging.info("  %s = %s", key, str(result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        if args.online == True:
            while True:
                print("input the sentence one: ")
                sentence1 = input()
                if sentence1 == "exit()":
                    break
                print("input the sentence two: ")
                sentence2 = input()
                if sentence2 == "exit()":
                    break
        ...
        #     attribute_candicate_sim = [(k, edit_distance(sentence, k)) for k in
        ...
        #     print("您需要查询的问题是：", result_df.question.loc[sim_question_index], "\t该问题的答案是：", answer)
