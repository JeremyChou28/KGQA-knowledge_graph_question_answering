...
def outline_KBQA(lines, results_path):
    ...
    for line in lines:
        try:
            total += 1
            question, entity, relation, answer, ner = line.split("\t")
            # answer = ''.join(answer.split())
            ner = ner.replace("#", "").replace("[UNK]", "%").replace("\n", "")
            # case: entity Fuzzy Match
            # 找出所有包含这些实体的三元组
            sql_e0_a1 = "select entity,relation,answer,question from webqa where entity like '%" + ner + "%' order by length(entity)"
            ...
            if len(result_e0_a1) == 0:
                ...
                sql_e0_a1 = "select entity,relation,answer,question from webqa where entity like '%" + linked_ner + "%' order by length(entity)"
                # sql查出来的是tuple，要转换成list才不会报错，每一个记录为[entity,attribute,answer,question]
            ...
            if len(candicate_triples) > 0:
                ...
                result_df = pd.DataFrame(candicate_triples,
                                         columns=['entity', 'relation', 'answer', 'question'])
                attribute_candicate_sim = []
                for index in range(len(result_df)):
                    h = result_df.loc[index]['entity']
                    k = result_df.loc[index]['relation']
                ...
                attribute_candicate_sort = sorted(attribute_candicate_sim, key=lambda candicate: candicate[3],
                                                  reverse=True)
                sim_question_index = \
                    result_df[result_df.relation == attribute_candicate_sort[0][1]].index.tolist()[0]
                ...
                if predict_answer == answer:
                    ...
                    semantic_true.append((question, entity, relation, answer))
                    loginfo.logger.info(
                    ...
                    accuracy = correct * 100.0 / total
                    semantic_error.append((question, entity, relation, answer, predict_answer))
                    loginfo.logger.info(
    ...
    df1.to_csv("../qaresults/webqa/" + results_path + "/semantic_match.csv", encoding='utf-8', index=False)

    df2 = pd.DataFrame(semantic_true, columns=["question", "entity", "relation", "answer"])
    df2.to_csv("../qaresults/webqa/" + results_path + "/semantic_true.csv", encoding='utf-8', index=False)

    df3 = pd.DataFrame(semantic_error, columns=["question", "entity", "relation", "answer", "predict_answer"])
    df3.to_csv("../qaresults/webqa/" + results_path + "/semantic_error.csv", encoding='utf-8', index=False)

    with open("../qaresults/webqa/" + results_path + "/semantic_examples.pkl", 'wb') as fo:
...
if __name__ == '__main__':
    ...
    loginfo = Logger("../log/webqa/Base5_kbqa.log", "info")
    line_list = read_file('../nerpredict/webqanerpredict/bert_softmax/q_t_a_testing_predict.txt')
    results_path = 'Base5'
    lines = []
    for i in range(len(line_list)):
        ...
        if ner == '\n':
            continue
        line = str(question) + '\t' + str(entity) + '\t' + str(attribute) + '\t' + str(answer) + '\t' + str(ner.strip())
        lines.append(line)
    print(len(lines))
