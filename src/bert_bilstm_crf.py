...
from lstm_crf_layer import BiLSTM_CRF  # 导入BiLSTM-CRF模型
...
flags.DEFINE_string(
    "bert_config_file", args.config_name,
...
flags.DEFINE_string("vocab_file", args.vocab_file,
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    bilstm_crf = BiLSTM_CRF(embedded_chars=embedding, hidden_unit=FLAGS.lstm_size, cell_type=FLAGS.cell,
                            num_layers=FLAGS.num_layers,
                            dropout_rate=FLAGS.dropout_rate, initializers=initializers, num_labels=num_labels,
                            seq_length=max_seq_length, labels=labels, lengths=lengths, is_training=is_training)
    rst = bilstm_crf.add_bilstm_crf_layer(crf_only=True)
    return rst
...
def main(_):
    ...
    # 在 train 的时候，才删除上一轮产出的文件，在 predicted 的时候不做clean
    ...
    # train_samples = processor._read_data(os.path.join(FLAGS.data_dir, "train.txt"))
    # dev_samples = processor._read_data(os.path.join(FLAGS.data_dir, "dev.txt"))
    # test_samples = processor._read_data(os.path.join(FLAGS.data_dir, "test.txt"))
    # print(len(train_samples))  # 11598
    # print(len(dev_samples))  # 2039
    # print(len(test_samples))  # 1864
    # print("train_samples:{}".format(train_samples))
    # print("dev_samples:{}".format(dev_samples))
    ...
    if FLAGS.do_eval:
        ...
        entity_eval = estimator.predict(input_fn=eval_input_fn)
        entity_eval_file = os.path.join(FLAGS.output_dir, "label_dev.txt")

        def eval_to_pair(writer):
            for predict_line, prediction in zip(eval_examples, entity_eval):
                idx = 0
                line = ''
        ...
        # 将模型预测的结果和原始标签写入到文件中，以空格分开，使用conlleval.py脚本来预测entity level 的结果并且输出
        with codecs.open(entity_eval_file, 'w', encoding='utf-8') as writer:
            eval_to_pair(writer)
        from conlleval import return_report
        entity_eval_result = return_report(entity_eval_file)
        print(''.join(entity_eval_result))
        with codecs.open(os.path.join(FLAGS.output_dir, 'entity_level_eval_result.txt'), 'a',
                         encoding='utf-8') as fd:
            fd.write(''.join(entity_eval_result))
