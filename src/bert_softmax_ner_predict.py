...
from bert_softmax import create_model, InputFeatures, InputExample
# from run_ner import create_model, InputFeatures, InputExample  # 基于BERT+BiLSTM+CRF的NER识别
from bert import tokenization  # 导入分词
from bert import modeling  # 导入bert模型
from bert_softmax import flags
...
def predict_outline(input_file, ner_predict_error_file, ner_predict_true_file, q_t_a_testing_predict_file):
    ...
    with graph.as_default():
        ...
        for row in nlpcc_test_data.index:
            ...
            attribute = str(nlpcc_test_data.loc[row, "attribute"])
...
if __name__ == "__main__":
    ...
    with graph.as_default():
        ...
        (total_loss, per_loss, logits, pred_ids) = create_model(
    ...
    if FLAGS.do_predict_outline:
        input_file = "../data/nlpcc2016/testing_clean_triple.csv"
        ner_predict_error_file = "../nerpredict/nlpcc2016nerpredict/ner_predict_error.txt"
        ner_predict_true_file = "../nerpredict/nlpcc2016nerpredict/ner_predict_true.txt"
        q_t_a_testing_predict_file = "../nerpredict/nlpcc2016nerpredict/q_t_a_testing_predict.txt"
    ...
    """
    # 单个question实体识别
    question = "koyo h414235/10轴承的型号是什么啊？"
